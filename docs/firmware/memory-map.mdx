...
| --- | --- | --- | --- | --- | --- |
| 0 | `0x3f400020` | 448,836 | DROM | R | `esp_app_desc_t`, then rodata: qstr pool, string constants, **frozen Python bytecode** |
...
- **Segments 2 and 4 are contiguous in DRAM.** `0x3ffbdb60 + 0x2688 = 0x3ffc01e8`
  (seg 4's start), so the two initialized-data segments form one continuous region
  even though the IROM segment sits between them in image order.
</Note>
