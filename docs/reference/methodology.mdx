...
(`LOAD_ATTR` order in the bytecode). The `CHUNK_HDR_FMT` name-to-value binding is
**inferred (high confidence)** — `f_ble/file_upload.py` does `from f_ble.chunking import FileTransfer, UPLOAD_TO_APP, CHUNK_HDR_FMT, CHUNK_HDR_SZ`,
so both names are real module-level exports of `chunking` consumed elsewhere (`CHUNK_HDR_SZ`
is used as the header size in the transfer loop), and `<HBBBiHiB` is `chunking`'s sole struct
format — but no literal `CHUNK_HDR_FMT = "…"` module-level assignment appears in the frozen
bytecode; the format is packed inline.
