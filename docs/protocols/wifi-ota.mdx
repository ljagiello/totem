...
<Steps>
  <Step title="Device polls for a release">
    ...
    ```json
    {"version": "5.0.2", "endpoint_id": 1, "device_type_id": 1,
     "lat": 0.0, "lon": 0.0, "gnss_time": 0, "release_id": 42}
    ```
  </Step>
...
digest only (confirmed in `f_ota/install_ota.py`); the ECDSA/mbedTLS machinery in the
image is unused here, so a custom OTA server needs no signing key.
</Warning>
