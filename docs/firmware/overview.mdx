...
| `version` | *(blank)* | field is empty (all-zero); the `5.0.2` version appears as a standalone string in DROM (a Python module constant), not in the OTA descriptor |
...
- An appended **SHA-256** (`hash_appended` set): `2a01e0f5…31058c` (valid), which the
...
<Note>
These three hashes are distinct: the **image-file** SHA-256 (`a6d05597…`, the whole `.bin`,
listed by the release API), the **appended validation** hash (`2a01e0f5…`, inside the image),
and the **app ELF** hash (`311224…`, in `esp_app_desc_t`, pinning the source ELF).
</Note>
