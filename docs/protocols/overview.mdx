...
| **`CHUNK_HDR_FMT` exact field order** | ✅ Recovered | wire format `<HBBBiHiB` (16 B) confirmed from live `struct.pack_into` in `chunking.gen_transfer_buff`; the names `CHUNK_HDR_FMT` / `CHUNK_HDR_SZ` also appear as importable symbols in `f_ble/file_upload.py`, so the name→value binding is high-confidence (no literal assignment survives in the frozen `chunking` module) |
...
reserved (B, packed as the literal `0`). The packed wire format and field order are
**confirmed** from the live `struct.pack_into`. The names `CHUNK_HDR_FMT` and
`CHUNK_HDR_SZ` are themselves real firmware symbols — `f_ble/file_upload.py` does
`from f_ble.chunking import … CHUNK_HDR_FMT, CHUNK_HDR_SZ` and reuses them in an identical
offset-2 `struct.pack_into` plus a `CHUNK_HDR_SZ`-length header slice — but the frozen
`chunking` module carries no module-level `CHUNK_HDR_FMT = …` assignment (it packs the
literal inline), so the `CHUNK_HDR_FMT` name→value binding stays **inferred (high
confidence)**.
...
`CHUNK_HDR_FMT` field order — have now been recovered by disassembling the frozen
MicroPython modules: `TOTEM_MSG_MAP` and its bindings live in `espnow_conn_v2.py`, and
`CHUNK_HDR_FMT` is packed in `f_ble/chunking.py` (see the tables above).
