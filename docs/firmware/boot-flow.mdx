...
feeds the native task watchdog (`mpy_machine_wdt` / `task_wdt`) only while the conditions are met
and no blocker is active (`[WDT {}] cond=[{}] blk=[{}]`). It supports dynamic threshold changes
