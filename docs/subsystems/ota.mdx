...
| Callbacks | `ota_callback.py` (`record_release`, `show_ota`), `ota_daemon.py` (`start_ota`, defaults `cmd=1, version='latest'`); `cb_start_ota` is a callback attribute wired on `compass` / `espnow_conn_v2`; `close_ota` (`f_ble/chunking.py`) and `Closing OTA writer` (`ota_block_writer.py`) close the writer |
...
the 96 frozen modules and are not part of `f_lib/firmware_rollback.py`; the module's
decision logic is carried in bytecode as shown above.
</Note>
...
- **App-level SHA-256 verification lives on the BLE transfer path**, not the WiFi install.
  ...
  disassembly `sha256*` appears **only** in `f_ble_chunking.dis`, `ota_ble.dis`, and
  `f_ble_file_upload.dis` — there is **no** SHA-256 in `f_ota/install_ota.py` or in the WiFi
  streaming writer `f_lib/firmware_ota.py`.
