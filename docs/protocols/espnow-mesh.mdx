...
The app never calls `wlan.config(lorate=...)`, so the LR data rate (`WIFI_PHY_RATE_LORA_250K`
vs `WIFI_PHY_RATE_LORA_500K`) is left at the ESP-IDF default (500K) — a client should leave
it unset too. *(PHY: Confirmed; lorate rate: Partial — IDF default, not set by the app.)*
