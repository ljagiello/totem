...
wraps a custom native driver `c_cap_touch` (python name `cap_touch`) built on the ESP-IDF
`touch_pad` API (`touch_pad_init failed`, etc.) — not the stock touch module.
...
values (`[touch] Low battery mode ON | base_sens: {:.3f}` then ` adjusted: {:.3f}`) via
`_LOW_BATT_FLOOR` and `_LOW_BATT_MULTIPLIER`.
